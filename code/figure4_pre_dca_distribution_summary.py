"""Composite incoming edge source-DCA distributions and Pre-DCA versus FCV panels."""
...
VALUES = TAB / "prepost_dca_signed_plus_minus_edge_fcv_node_values.csv"
PRE_SAMPLE = TAB / "pre_edge_source_dca_distribution_sample_by_unit.csv"
...
OUT = FIG / "pre_edge_source_dca_distribution_and_predca_fcv_summary.png"
OUT_STATS = TAB / "pre_edge_source_dca_distribution_and_predca_fcv_summary_stats.csv"
...
def distribution_limits(sub: pd.DataFrame, samp: pd.DataFrame, species: str) -> tuple[float, float]:
    vals_all = samp["source_DCA"].to_numpy(float)
    ...
    anchors = sub[["PreDCA", "edge_source_dca_q25", "edge_source_dca_q75"]].to_numpy(float).ravel()
...
def plot_distribution(
    ...
    plot_samp = samp.dropna(subset=["x", "source_DCA"]).copy()
    plot_samp = plot_samp[(plot_samp["source_DCA"] >= ymin) & (plot_samp["source_DCA"] <= ymax)]
    ...
    for class_order, group in plot_samp.groupby("class_order", sort=True):
        ...
        ax.scatter(
            ...
            group["source_DCA"].to_numpy(float),
    ...
    for _, row in sub.iterrows():
        x0 = float(row["x"])
        q25 = np.clip(row["edge_source_dca_q25"], ymin, ymax)
        q75 = np.clip(row["edge_source_dca_q75"], ymin, ymax)
        ax.add_patch(
            plt.Rectangle(
        ...
        if np.isfinite(mean_value):
            ...
            ax.scatter(
                ...
                s=26,
                marker="D",
                color=fs.MAIN_COLORS["pre_in"],
                edgecolor="white",
                linewidth=0.45,
                zorder=6,
            )
    if mean_x:
        ax.plot(mean_x, mean_y, color=fs.MAIN_COLORS["pre_in"], lw=1.15, alpha=0.9, zorder=5)

    unit = "neurons" if species == "C. elegans" else "regions"
    ax.axhline(0, color="#777777", lw=0.7, ls="--")
    ax.set_title(f"{species}: incoming edge source-DCA distribution", fontsize=10, pad=2)
    ax.set_xlabel(f"Target {unit} sorted by z-FCV", fontsize=8.5)
    ax.set_ylabel("Source DCA", fontsize=8.5)
...
def plot_scatter(ax: plt.Axes, values: pd.DataFrame, stats: pd.DataFrame, species: str, panel_label: str) -> None:
    sub = (
        values.loc[values["species"].eq(species), ["PreDCA", "EdgeStdFCV"]]
        .replace([np.inf, -np.inf], np.nan)
        .dropna()
    )
    sub = add_functional_classes(values.loc[values["species"].eq(species), ["species", "node", "PreDCA", "EdgeStdFCV"]])
    sub = sub.replace([np.inf, -np.inf], np.nan).dropna(subset=["PreDCA", "EdgeStdFCV"])
    x = sub["PreDCA"].to_numpy(float)
    y = sub["EdgeStdFCV"].to_numpy(float)

    for class_order, group in sub.groupby("class_order", sort=True):
        key = int(class_order)
        ax.scatter(
            group["PreDCA"].to_numpy(float),
    ...
    ax.set_xlabel("Pre-DCA", fontsize=9)
...
def main() -> None:
    ...
    values = replace_fcv_with_recording_zscore(pd.read_csv(VALUES))
    stats = pd.DataFrame([trend_stats(values, species, "PreDCA") for species in SPECIES])
    stats.to_csv(OUT_STATS, index=False)
    sample = pd.read_csv(PRE_SAMPLE)
    summary = replace_fcv_with_recording_zscore(pd.read_csv(PRE_SUMMARY))
    predca = values[["species", "node", "PreDCA"]].drop_duplicates(["species", "node"])
    summary = summary.drop(columns=["PreDCA"], errors="ignore").merge(predca, on=["species", "node"], how="left")
