...
| `figure14_15_celegans_drosophila_AFHI_combined.py` | `figures/figure14_15_celegans_drosophila_AFHI_combined.png` |
| `figure7_by_species_with_figure5_abc.py` | `figures/figure7_by_species_with_figure5_abc.png` |

## Supplementary Figure Scripts

Supplementary scripts use two historical working-output conventions. Some write PNGs to `figures/`; others write PNGs to `output/png/`. Running `scripts/sync_outputs.sh` stages every supplementary PNG into `figures/` and then copies the clean final PNG set to `outputs/supplementary/`. PDF files are not retained in this pack.
...
- `figure*_*.py` helper modules: cross-species and simulation subpanels imported by the main scripts.
